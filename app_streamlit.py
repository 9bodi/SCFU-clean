...
<style>
    ...
    * { font-family: 'Inter', sans-serif; }
    ...
    .stButton > button {
        background: linear-gradient(135deg, #00ff87 0%, #60efff 100%);
        color: #0f1419; border: none; border-radius: 12px;
    ...
    .stDownloadButton > button {
        ...
        padding: 0.6rem 1.8rem; font-weight: 600; font-size: 0.9rem;
    ...
    /* ===== Sidebar contrast fix (labels en blanc) ===== */
    ...
    .player-section {
        ...
        border-image: linear-gradient(180deg, #00ff87 0%, #60efff 100%) 1;
    ...
    .info-badge {
        ...
        color: #60efff; border: 1px solid rgba(96, 239, 255, 0.3);
        margin: 0.3rem;
    }
    ...
    .pressure-indicator {
        ...
        border-left: 4px solid #ff6b9d; border-radius: 8px; margin-top: 1rem;
        font-size: 0.95rem; color: #ffb3d9;
    }
...
def create_comparison_chart(
    ...
    fig.add_trace(
        go.Bar(x=player_names, y=poss_vals, showlegend=False, text=[f"{v:.1f}%" for v in poss_vals], textposition="outside"),
        row=1,
        col=1,
    )
    ...
    fig.add_trace(
        go.Bar(x=player_names, y=struct_vals, showlegend=False, text=[f"{v:+.1f}%" for v in struct_vals], textposition="outside"),
        row=1,
        col=2,
    )
    ...
    fig.add_trace(
        go.Bar(x=player_names, y=tempo_vals, showlegend=False, text=[f"{v:+.1f}%" for v in tempo_vals], textposition="outside"),
        row=2,
        col=1,
    )
    ...
    press_vals = []
    for s in summaries:
        p1 = get_summary_value(s, "pressure_n_r1_mean", [])
        p2 = get_summary_value(s, "pressure_n_r2_mean", [])
    ...
    fig.add_trace(
        go.Bar(x=player_names, y=press_vals, showlegend=False, text=[f"{v:.1f}" for v in press_vals], textposition="outside"),
        row=2,
        col=2,
    )
...
if matches_list:
    ...
    if not (events_csv and tracking_jsonl):
        st.error("⚠️ Fichiers du match incomplets")
        st.stop()
    ...
    if not teams:
        st.warning("Aucune équipe détectée dans les events.")
        st.stop()
    ...
    <div style="text-align: center; padding: 3rem 1rem;">
        ...
        <p style="font-size: 1.1rem; color: rgba(255,255,255,0.75); max-width: 600px; margin: 0 auto 2rem;">
